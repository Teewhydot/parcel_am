...
class _LoginScreenState extends State<LoginScreen>
  ...
  void codeUpdated() {
    ...
    if (code != null && code!.length == 6) {
      ...
      if (state.isOtpSent && !state.isLoading) {
        _verifyOTP();
      }
    }
  }
      ...
      builder: (context, state) {
        return SingleChildScrollView(
          child: Column(
            ...
            // OTP Input
            AppText.bodyMedium('Verification Code', fontWeight: FontWeight.w500),
            AppSpacing.verticalSM,
            ...
            Center(
              child: TextButton(
                onPressed: state.resendCooldown > 0 || state.isLoading ? null : _sendOTP,
                child: AppText.bodySmall(
                  state.resendCooldown > 0 
                      ? 'Resend code in ${state.resendCooldown}s'
                      : 'Resend verification code',
                  color: state.resendCooldown > 0 ? Colors.grey : AppColors.primary,
                ),
              ),
            ),
              ...
              Center(
                child: TextButton(
                  onPressed: state.isLoading ? null : () {
                    _resetOTPState();
                    context.read<AuthBloc>().add(AuthPhoneNumberChanged(_phoneController.text));
                  },
                  child: AppText.bodySmall(
                    'Change phone number',
                    color: AppColors.primary,
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: state.isLoading 
                  ? null 
                      : (state.isOtpSent ? _verifyOTP : _sendOTP),
              ...
              child: state.isLoading
                  ...
                  : AppText.bodyLarge(
                      state.isOtpSent ? 'Verify Code' : 'Send Verification Code',
                      color: Colors.white,
                      fontWeight: FontWeight.w600,
                    ),
                ),
        ...
        return SingleChildScrollView(
          child: Column(
            ...
            AppContainer(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  AppText.bodySmall('Creating account for:', color: Colors.grey),
                  AppSpacing.verticalXS,
                  AppText.bodyMedium(
                    '${_firstNameController.text} ${_lastNameController.text}',
                    fontWeight: FontWeight.w500,
                  ),
                  AppText.bodySmall(_phoneController.text, color: Colors.grey),
                ],
              ),
            ),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: state.isLoading 
                  ? null 
                  : (state.isOtpSent ? _verifyOTP : () => _signUpWithPhone()),
              style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.primary,
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(12),
              ...
              child: state.isLoading
                  ...
                  : AppText.bodyLarge(
                      state.isOtpSent ? 'Create Account' : 'Send Verification Code',
                      color: Colors.white,
                      fontWeight: FontWeight.w600,
                    ),
                ),
